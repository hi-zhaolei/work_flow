...
(function() {
  (function(Z) {
    if (window.$ && window.jQuery) {
      return $.getScript('./dom.jsPlumb-1.6.2.js', function() {
        return Z.init();
      });
    } else {
    ...
    init: function() {
      window.jsPlumb_Z_config && $.extend(this.config, window.jsPlumb_Z_config);
      this.config.debug && (window.jsPlumb_Z = this);
      $("#jsPlumb_demo").html('');
      if (this.config.display === 'workflow') {
        return this.initWorkFlow();
      } else if (this.config.display === 'rely') {
    ...
    initWorkFlow: function() {
      ...
      if (this.config.type === 'XML') {
        ...
        return jsPlumb.ready((function(_this) {
          return function() {
            _this.initJsPlumb();
            return _this.storage.isInit = true;
          };
        })(this));
      } else if (this.config.type === 'ajax') {
        return this.getAjax((function(_this) {
          return function() {
            _this.render(_this.DATA = arguments[0]);
    ...
    initRelation: function() {
      var _this;
      this.initRelationDom().initCss().util.radius = this.config.radius;
      _this = this;
      ...
      return this.getAjax(function() {
        this.renderRelation(this.DATA = arguments[0]);
        return jsPlumb.ready((function(_this) {
          return function() {
            return _this.initJsPlumbRely();
          };
        })(this));
      });
    },
    ...
    initEvent: function() {
      ...
      $('#jsPlumb_line').on('click', function() {
        ...
        if ($(this).hasClass('active')) {
          ...
          jsPlumb.draggable($("#jsPlumb_demo_ctn .w"));
          jsPlumb.unmakeEverySource($("#jsPlumb_demo_ctn .w"));
        } else {
          $(this).addClass('active');
          jsPlumb.destroyEveryDraggable($('#jsPlumb_demo_ctn .w'));
          jsPlumb.makeSource($("#jsPlumb_demo_ctn .w").not('#2'), {
            anchor: "Continuous",
      ...
      $('#jsPlumb_demo').attr('tabindex', 1).on('keydown', (function(_this) {
        return function(ev) {
          var id, keymap;
          if (_this.storage.isEdit) {
            if (ev.keyCode === 13) {
        ...
        if (confirm('数据将提交给后台，确认继续，取消将中断该操作')) {
          if (_this.config.submitBefore && _this.config.submitBefore(json)) {
            return $.post(this.config.submitUrl, {
              ...
              if (+data) {
                alert('数据提交成功！');
                return _this.config.submitAfter && _this.config.submitAfter(json);
              } else {
                return alert(data);
              }
            });
    ...
    render: function() {
      ...
      (function(data, i, params) {
        ...
        w = data[i];
        if (i in sourceMap) {
          sourceMap[i].indexOf(params.p) === -1 && sourceMap[i].push(params.p);
          _w = $('#' + w.id);
          t = 0;
          l = [];
          _ref = sourceMap[i];
        ...
        if (node["in"]) {
          if (node.out) {
            ...
            for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
              _in = _ref[i];
              _in === node.out[0] && node["in"].splice(i, 1);
            }
          }
    ...
    initJsPlumb: function() {
      ...
      jsPlumb_work = jsPlumb.getInstance({
        anchor: "Continuous",
        connector: ["StateMachine"],
        PaintStyle: {
          strokeStyle: "#5c96bc",
    ...
    initJsPlumbRely: function() {
      ...
      jsPlumb_rely = jsPlumb.getInstance({
        anchor: "Continuous",
        connector: ["StateMachine"],
        PaintStyle: {
          strokeStyle: "#5c96bc",
